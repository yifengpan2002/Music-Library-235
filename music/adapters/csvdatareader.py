...
class TrackCSVReader:
    ...
    def read_albums_file_as_dict(self) -> dict:
        ...
        root_DIR = root_DIR + "/data/" + self.__albums_csv_file
        if not os.path.exists(self.__albums_csv_file): #root_dir can be replaced by self.__albums_csv_file
            print(f"path {self.__albums_csv_file} does not exist!")

        album_dict = dict()
        # encoding of unicode_escape is required to decode successfully
        with open(self.__albums_csv_file, encoding="unicode_escape") as albums_csv:
            reader = csv.DictReader(albums_csv)
            for row in reader:
                album_id = int(
    ...
    def read_tracks_file(self):
        root_DIR = os.path.dirname(os.path.abspath(__file__))
        root_DIR = root_DIR + "/data/" + self.__tracks_csv_file
        if not os.path.exists(self.__tracks_csv_file):  # root_dir can be replaced by self.__albums_csv_file
